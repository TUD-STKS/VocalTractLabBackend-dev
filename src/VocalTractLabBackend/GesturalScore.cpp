...
bool GesturalScore::deleteGesture(int gestureType, int gestureIndex)
{
	...
	{
		const double duration_s = sequence->getGesture(gestureIndex)->duration_s;
...
int GesturalScore::insertGesture(int gestureType, double insertPos_s, int gestureIndex)
{
	GestureSequence* sequence = &gestures[gestureType];
	gestureIndex = sequence->getIndexAt(insertPos_s);
	...
	if (sequence->isValidIndex(gestureIndex))
	{
		...
		gesture->duration_s = startPos_s + gesture->duration_s - insertPos_s;

		Gesture newGesture = *gesture;
		newGesture.duration_s = insertPos_s - startPos_s;
		...
		// ****************************************************************
	{
		...
		newGesture.duration_s = insertPos_s - sequence->getDuration_s();
...
bool GesturalScore::setGestureSlope(int gestureType, int gestureIndex, double slope)
{
	...
	auto* g = s->getGesture(gestureIndex);
	const auto newSlope = s->minSlope + slope * (s->maxSlope - s->minSlope);
	if (abs(newSlope - g->slope) < 0.0001)
	{
		return false;
	}
	g->slope = newSlope;
...
bool GesturalScore::setGestureTimeConstant(int gestureType, int gestureIndex, double timeConstant)
{
	...
	auto* g = s->getGesture(gestureIndex);
	const auto newTau = s->minTau_s + timeConstant * (s->maxTau_s - s->minTau_s);
	if(abs(newTau - g->tau_s) < 0.0001)
	{
		return false;
	}
	g->tau_s = newTau;
...
void GesturalScore::setGlottis(Glottis* newGlottis)
{
	glottis = newGlottis;
}

VocalTract* GesturalScore::getVocalTract() const
{
	return vocalTract;
}

void GesturalScore::setVocalTract(VocalTract* newVocalTract)
{
	vocalTract = newVocalTract;
}

vector<Target>* GesturalScore::getTractParamTargets()
{
	return tractParamTargets;
}

vector<Target>* GesturalScore::getGlottisParamTargets()
{
	return glottisParamTargets;
}

vector<double>* GesturalScore::getTractParamCurve()
{
	return tractParamCurve;
}

vector<double>* GesturalScore::getGlottisParamCurve()
{
	return glottisParamCurve;
}

GestureSequence* GesturalScore::getGestures()
{
	return gestures;
}

Glottis* GesturalScore::getGlottis() const
{
	return glottis;
}
