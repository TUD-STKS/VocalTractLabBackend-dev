	...
	double freq, double direction, enum physicalQuantity quant)
{
	...
	else
	{
		...
		for (int i(0); i < numX - 1; i++)
		{
			...
			if (dX < 0.) {
				l0 = scaling(((double)(numX - i)-1.5 + sqrt(3)/6.)/(double)(numX - 1));
			}
			else
			{
				l0 = scaling((double)(i + 0.5 - sqrt(3) / 6.) / (double)(numX - 1));
			}
			...
			if (dX < 0.)
			{
				l1 = scaling(((double)(numX - i) - 1.5 - sqrt(3) / 6.) / (double)(numX - 1));
			}
			else
			{
				l1 = scaling((double)(i + 0.5 + sqrt(3) / 6.) / (double)(numX - 1));
			}
			log << "dl = " << dl << " l0 = " << l0 << " l1 = " << l1 
				<< " K2(1,1) " << K2(0,0) << endl;
			// build matrix K
			K2.setZero(mn, mn);
			for (int j(0); j < mn; j++)
